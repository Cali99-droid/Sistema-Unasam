...
$sql = $db->query("SELECT gu.nombre_grupo, count(*) tendencia
      from alumno a
      ...
      inner join grupo_universitario gu on gu.idgrupo_universitario=ag.idgrupo_universitario
      inner join tipo_grupo tg on tg.idTipoGrupo=gu.idTipoGrupo
      group by gu.idgrupo_universitario order by tendencia desc;");

while ($empleados = $sql->fetch_assoc()) {
    //cambio de color
    ...
    $pdf->Cell(100, 10, $empleados['nombre_grupo'], 1, 0, 'C', 1);
    /* $pdf->Cell(30,10,$empleados['apellidos'],1,0,'C',1);
            $pdf->Cell(30,10,$empleados['ApellidoMater'],1,0,'C',1);
            $pdf->Cell(56,10,utf8_decode($empleados['Titulo']),1,0,'C',1);
            $pdf->Cell(25,10,$empleados['Nom_Tipo'],1,0,'C',1);]*/

    $pdf->Cell(30, 10, utf8_decode($empleados['tendencia']), 1, 1, 'C', 1);
}
$pdf->Output();
